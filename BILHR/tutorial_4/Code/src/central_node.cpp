...
// Global Variables
enum robot_state {standby,training,executing,recording};
...
static const char PATH_TRAINING_DATA_75[] = "TrackedPositions75.txt";
static const char PATH_TRAINING_DATA_150[] = "TrackedPositions150.txt";
static const char PATH_TRAINING_DATA_600[] = "TrackedPositions600.txt";
static const char PATH_TRAINING_DATA_1000[] = "TrackedPositions1000.txt";
static const char PATH_TRAINING_DATA_1300[] = "TrackedPositions1300.txt";
static const char PATH_TRAINING_DATA_2000[] = "TrackedPositions2000.txt";
...
//double filtered_ball_coordinates[2];
ofstream myfile;
...
void visionCB(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    GaussianBlur(in_image, img_smoothed, Size(11,11),0,0);

    // convert image to HSV color space
    cvtColor(img_smoothed, hsv_image, CV_BGR2HSV);
    //cvtColor(in_image, hsv_image, CV_BGR2HSV);

    // threshold image to detect colors
    // Blue
    cv::inRange(hsv_image, cv::Scalar(90,90,70), cv::Scalar(110,255,255), img_thresholded);
    ...
    erode(img_thresholded,mor_img,Kernel,Point(-1,-1),1);
    dilate(mor_img,mor_img,Kernel,Point(-1,-1),1);

    GaussianBlur(mor_img, img_aux, Size(11,11),0,0);

    vector<Vec3f> circles;
    HoughCircles(img_aux, circles, CV_HOUGH_GRADIENT, 1, img_aux.rows/4, 100, 20, 20, 80);

    if(circles.size()>0){
        circle( in_image, cvPoint(circles[0][0],circles[0][1]),3,Scalar(0,255,255),-1,8,0);
        ball_coordinates[0] = circles[0][0];
        ball_coordinates[1] = circles[0][1];
    }
    ...
    if(count > 0)
    {
        ball_coordinates[0] = sum_x / count;
        ball_coordinates[1] = sum_y / count;
    }*/

    // send a simple messege with x and y coordinate of the detected blob
    BILHR_ros::Detection detection_msg;
    detection_msg.x = ball_coordinates[0];
    detection_msg.y = ball_coordinates[1];

    detection_pub.publish(detection_msg);

    //tried filtering
    //filter->filter( ball_coordinates, filtered_ball_coordinates);
    //circle(in_image, cvPoint(filtered_ball_coordinates[0], filtered_ball_coordinates[1]), 5, CV_RGB(255,0,0),3);

    // draw a red circle in the center of the detected blob (in the input image)
    //circle(in_image, cvPoint(ball_coordinates[0], ball_coordinates[1]), 5, CV_RGB(255,0,0),3);
...
void tactileCB(const BILHR_ros::TactileTouch::ConstPtr& __tactile_touch)
{
    ...
    if (((int)__tactile_touch->button == 3) && ((int)__tactile_touch->state == 1))
    {
        if(state == training)
        {
            if(sample_counter == 0)
            {
                SaySomething("Loading standard training data with 1300 samples");

                cout << "Entered Learning mode" << endl;
                net->load_training_data(PATH_TRAINING_DATA_75);
    ...
    if (((int)__tactile_touch->button == 2) && ((int)__tactile_touch->state == 1))
    {
        if(state == training)
        {
            if(sample_counter == 0)
            {
                SaySomething("Loading standard training data with 600 samples");

                cout << "Entered Learning mode" << endl;
                net->load_training_data(PATH_TRAINING_DATA_600);
...
*/
    ...
    if (state == recording){
        ros::Duration waitShort(0.05);

        if(ball_coordinates[0]!=0 && ball_coordinates[1]!=0){
            double pos_s_pitch = motor_r_arm_in[0];
            double pos_s_roll = motor_r_arm_in[1];

            myfile.open("TrackedPositions.txt",ios::ate|ios::in);
            myfile << ball_coordinates[0] << "\t" << ball_coordinates[1] << "\t" << pos_s_pitch << "\t" << pos_s_roll << "\n";
            myfile.close();

            cout << "Data was successfully saved " << sample_counter <<endl;
            cout << "ball [0]: " << ball_coordinates[0] << " ball[1]: " << ball_coordinates[1] <<endl;
    ...
    if (state == executing){
        ...
        double ball1 = 0, ball0 = 0;
        cout << "ball [0]: " << ball_coordinates[0] << " ball[1]: " << ball_coordinates[1] <<endl;

        if( abs(ball_coordinates[0]-ball0) > 5 || abs(ball_coordinates[1]-ball1) > 5 ){
            net->CMAC_Map(ball_coordinates, desired_position);
        }

        cout << "des[0]: " << desired_position[0] << endl;
        cout << "des[1]: " << desired_position[1] << endl;

        if (desired_position[0] != 0 && desired_position[1] != 0){
...
int main(int argc, char** argv)
{
    ...
    //filter = new Butter2Order(2, 1.0, -1.561018075800718, 0.641351538057563, 0.020083365564211, 0.040166731128423, 0.020083365564211);
