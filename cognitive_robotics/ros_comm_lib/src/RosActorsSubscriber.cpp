#include "RosActorsSubscriber.h"


namespace comminterface
{

RosActorsSubscriber::RosActorsSubscriber(const std::string& topicName)
{
    mTopicName = topicName;

    _sub = n.subscribe(topicName, 1000, &RosActorsSubscriber::actorCallback, this);

    runThread = true;
}

RosActorsSubscriber::~RosActorsSubscriber()
{
...
void RosActorsSubscriber::actorCallback(const ros_tum_msgs::ActorVecConstPtr& data)
{
    //std::cout << "actorCallback" << std::endl;
    this->setActorsData(data);
}

void RosActorsSubscriber::setActorsData(const ros_tum_msgs::ActorVecConstPtr& data)
{
...
void RosActorsSubscriber::getActorsData(ros_tum_msgs::ActorVec& data)
{
